...
export class SocketServiceSingleton {
  ...
  private _teamInfo$: BehaviorSubject<TeamInfo | null | undefined> = new BehaviorSubject<TeamInfo | null | undefined>(
    undefined
  )
  ...
  problems(): Observable<Problem[] | undefined> {
    return this._problems$.pipe(debounceTime(1000))
  }
    ...
    switch (event.event) {
      ...
      case 'INSERT_PROBLEM':
        if (event.data.before) {
          this._problems.splice(
            this._problems.findIndex((problem) => problem.id === event.data.before),
            0,
            { ...event.data, before: undefined }
          )
        } else {
